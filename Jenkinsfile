...
def build_jvm_windows() {
  ...
  dir('packages') {
      bat "cd cinterop\\src\\jvmMain\\windows && if exist build-dir rmdir /s /q build-dir && mkdir build-dir && cd build-dir &&  \"${tool 'cmake'}\" ${cmakeDefinitions} .. && \"${tool 'cmake'}\" --build . --config Release"
  }
