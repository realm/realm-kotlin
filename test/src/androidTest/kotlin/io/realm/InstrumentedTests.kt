...
class InstrumentedTests {
    ...
    fun garbageCollectorShouldFreeNativeResources() {
        ...
        mappedMemorySize = numberOfMemoryMappedBytes(command)
        assertTrue(mappedMemorySize >= 99 * oneMB, "Realm and its memory should still be allocated since we didn't release all the inserted objects yet. Current amount is ${bytesToHumanReadable(mappedMemorySize)}")
    ...
    fun closeShouldFreeMemory() {
        ...
        mappedMemorySize = numberOfMemoryMappedBytes(command)
        assertTrue(mappedMemorySize >= 99 * oneMB, "Committing the 100 objects should result in memory mapping of ~ 99 MB. Current amount is ${bytesToHumanReadable(mappedMemorySize)}")
