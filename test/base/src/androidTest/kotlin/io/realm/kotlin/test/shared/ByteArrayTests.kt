...
class ByteArrayTests {
    ...
    @Test
    fun asd() {
        val byteArray = ByteArray(1).apply {
            set(0, 1)
        }
        roundTrip(byteArray) { value ->
            assertEquals(byteArray, value)
        }
    }

    // Store value and retrieve it again
    private fun roundTrip(byteArray: ByteArray, function: (ByteArray) -> Unit) {
        // Test managed objects
        realm.writeBlocking {
            copyToRealm(
                Sample().apply {
                    binaryField = byteArray
                }
            )
