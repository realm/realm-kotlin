...
class CopyFromRealmTests {
    ...
    fun simpleValues() {
        ...
        val unmanagedObj = insertedObj.copyFromRealm<Sample>()
    ...
    fun primitiveLists() {
        ...
        schemaProperties.forEach { prop: RealmProperty ->
            if (prop.type is ListPropertyType) {
                ...
                val type = accessor.returnType.toString()
                val t = type.removePrefix("io.realm.kotlin.types.RealmList<").removeSuffix(">").removeSuffix("?")
                val list: MutableList<Any?> = when(t) {
                    ...
                    "io.realm.kotlin.entities.Sample" -> realmListOf()
                    else -> TODO()
                }
                if (prop.isNullable) {
                    list.add(null)
                }
                accessor.set(realmObject, list)
            }
        }

        val unmanagedObj = realm.writeBlocking {
            copyToRealm(realmObject).copyFromRealm<Sample>()
        }
        ...
        schemaProperties.forEach { prop: RealmProperty ->
            if (prop.type is ListPropertyType) {
                ...
                if (prop.type.storageType == RealmStorageType.BINARY) {
                    if (prop.isNullable) {
                        val copy = accessor.get(unmanagedObj) as List<ByteArray?>
                    ...
                    assertContentEquals(list, accessor.get(unmanagedObj) as List<Any?>, "${prop.name} failed")
                }
            }
        }
    }
