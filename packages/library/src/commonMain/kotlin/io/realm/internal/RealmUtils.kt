...
internal inline fun checkRealmClosed(realm: NativePointer?, configuration: RealmConfiguration? = null) {
    if (realm != null && RealmInterop.realm_is_closed(realm)) {
...
fun <T : RealmObject> create(mediator: Mediator, realm: RealmId, type: KClass<T>): T {
...
@Suppress("TooGenericExceptionCaught") // Remove when errors are properly typed in https://github.com/realm/realm-kotlin/issues/70
fun <T : RealmObject> create(mediator: Mediator, realm: RealmId, type: KClass<T>, primaryKey: Any?): T {
    // FIXME Does not work with obfuscation. We should probably supply the static meta data through
    //  the companion (accessible through schema) or might even have a cached version of the key in
    //  some runtime container of an open realm.
    //  https://github.com/realm/realm-kotlin/issues/85
...
fun <T : RealmObject> copyToRealm(mediator: Mediator, realm: RealmId, instance: T, cache: MutableMap<RealmObjectInternal, RealmObjectInternal> = mutableMapOf()): T {
