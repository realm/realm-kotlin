...
internal class CountQuery constructor(
    ...
    private var previousCount: Long = -1
    ...
    override fun Flow<BaseResults<Long>?>.queryMapper(): Flow<Long> = this.map {
        ...
        discardRepeated(latestCount)
    }

    override fun discardRepeated(latestValue: Long?): Boolean {
        return if (previousCount == latestValue) {
            false
        } else {
            previousCount = latestValue!!
            true
        }
    }
}
...
internal class GenericAggregatorQuery<T : Any> constructor(
    ...
    private var firstEmission: Boolean = true
    ...
    override fun Flow<BaseResults<T>?>.queryMapper(): Flow<T?> = this.map {
        it?.let { results -> findFromResults(results.nativePointer) }
    }.filter { latestValue: T? ->
        discardRepeated(latestValue)
    }

    override fun discardRepeated(latestValue: T?): Boolean {
        return if (firstEmission) {
            firstEmission = false
...
//    type: KClass<T>
//): T? {
//    val average = RealmInterop.realm_results_average<Double?>(resultsPointer, colKey) ?: return null
//
//    // TODO Expand to support other numeric types, e.g. Decimal128
//    return when (type) {
