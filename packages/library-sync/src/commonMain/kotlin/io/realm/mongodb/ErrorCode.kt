...
enum class ErrorCode constructor(
     ...
     * have access to this Realm. Check that the [SyncConfiguration] is correct.
     *
     *
     ...
     */
    val category: Category = Category.FATAL
) {
    ...
    NETWORK_IO_EXCEPTION(Type.JAVA, 0 /*OsJavaNetworkTransport.ERROR_IO*/),
    NETWORK_INTERRUPTED(Type.JAVA, 1 /*OsJavaNetworkTransport.ERROR_INTERRUPTED*/),
    NETWORK_UNKNOWN(Type.JAVA, 2 /*OsJavaNetworkTransport.ERROR_UNKNOWN*/),
     ...
     * identified by the `(type:value)` pair.
     *
     * @return the error code as an unique `int` value.
     */
    fun intValue(): Int {
        return code
    }
    ...
    object Type {
        ...
        const val AUTH = "auth" // Errors from the Realm Object Server
        const val CONNECTION = "realm.basic_system" // Connection/System errors from the native Sync Client
        const val DEPRECATED = "deprecated" // Deprecated errors
        const val HTTP = "realm::app::HttpError" // Errors from the HTTP layer
        const val JAVA = "realm::app::CustomError" // Errors from the Java layer
        const val MISC = "realm.util.misc_ext" // Misc errors from the native Sync Client
        const val SERVICE = "realm::app::ServiceError" // MongoDB Realm Response errors
        const val JSON = "realm::app::JSONError" // Errors when parsing JSON
        const val PROTOCOL = "realm::sync::ProtocolError" // Protocol level errors from the native Sync Client
        const val SESSION = "realm::sync::Client::Error" // Session level errors from the native Sync Client
        const val UNKNOWN = "unknown" // Catch-all category
    }
    ...
    companion object {
         ...
         * a Java error does not exists. [.UNKNOWN] will be returned.
         */
        internal fun fromNativeError(type: String, errorCode: Int): ErrorCode {
            val errorCodes = values()
            for (i in errorCodes.indices) {
                val error = errorCodes[i]
                if (error.intValue() == errorCode && error.type == type) {
                    return error
                }
            }
            // FIXME
