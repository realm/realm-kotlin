...
### Breaking Changes
* Introduced `BaseRealmObject` as base interface of `RealmObject` and updated most APIs to accept `BaseRealmObject` instead of `RealmObject` to prepare for future embedded object support. This doesn't require code changes even though the API has changed. 
